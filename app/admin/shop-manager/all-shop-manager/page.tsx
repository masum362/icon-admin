...
const AllShopManager = () => {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this shop manager?")) {
      try {
        await deleteUser(id);
        toast({
          title: "Shop manager deleted successfully",
          description: "The shop manager has been removed from the system",
        });
        refetch(); // Refresh the list
      } catch (error: any) {
        toast({
          variant: "destructive",
          title: "Error deleting shop manager",
    ...
    <div className="main-content group-data-[sidebar-size=lg]:xl:ml-[16px] group-data-[sidebar-size=sm]:xl:ml-[16px] px-4 group-data-[theme-width=box]:xl:px-0 ac-transition">
      <div className="card p-0 lg:min-h-[calc(100vh_-_theme('spacing.header')_*_1.4)] xl:min-h-[calc(100vh_-_theme('spacing.header')_*_1.6)]">
        <div className="flex flex-col gap-2 sm:flex-center-between sm:flex-row px-4 py-5 sm:p-7 bg-gray-200/30 dark:bg-dark-card-shade">
          <div>
            <h6 className="card-title">Shop Manager list</h6>
            <p className="card-description">All Shop Managers Here</p>
          </div>
          <Link
        ...
        {/* Start All Shop Manager List Table */}
              ...
              <>
                <table className="table-auto w-full whitespace-nowrap text-left text-gray-500 dark:text-dark-text leading-none">
                  <thead className="border-b border-gray-200 dark:border-dark-border font-semibold">
                    <tr>
                      <th className="px-4 py-4">Shop Manager</th>
                        ...
                        >
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-3.5">
                              <Link
                                href={`/admin/shop-manager/${user._id}`}
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <Link
                                href={`/admin/shop-manager/create-shop-manager?id=${user._id}`}
        ...
        {/* End All Shop Manager List Table */}
      </div>
    </div>
  );
};

export default AllShopManager;

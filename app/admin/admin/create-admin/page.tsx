...
const CreateAdminForm = () => {
  ...
  useEffect(() => {
    if (adminData) {
      ...
      if (adminData.profile_picture) {
        setProfileImageUrl(adminData.profile_picture);
      }

      // Set editor content if available
      if (adminData.metaDescription && editor) {
        editor.commands.setContent(adminData.metaDescription);
      }
    }
  }, [adminData, form, editor]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (adminId) {
        await axiosInstance.patch(`/admin/${adminId}`, adminData);
        toast({
          title: "Admin updated successfully",
          description: "The admin information has been updated",
        });
      } else {
        await axiosInstance.post("/user/create-admin", adminData);
        toast({
      ...
      router.push("/admin/admin/all-admin");
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: adminId ? "Error updating admin" : "Error creating admin",
